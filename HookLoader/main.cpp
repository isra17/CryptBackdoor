...
const wchar_t kUsage[] =
    L"Usage: HookLoader [-p pid] [-n name] [pids...] hooked_dll";
...
struct Options {
  ...
  static Options Parse(int argc, wchar_t** argv) {
    ...
    if (!FileExists(options.hookedDllPath)) {
      wcerr << "Hooked dll '" << options.hookedDllPath << "' does not exist"
            << endl;
      exit(1);
    }
    argv[argc - 1] = 0;
...
void HookProcess(int pid, const wstring& hookedDllPath) {
  ...
  LPVOID pLoadLibraryW =
      (LPVOID)GetProcAddress(GetModuleHandle(L"kernel32.dll"), "LoadLibraryW");
  assert(pLoadLibraryW);

  DWORD pathSize = (hookedDllPath.length() * 2 + 1);
  LPVOID lpParam = (LPVOID)VirtualAllocEx(
      hProcess, NULL, pathSize, MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);
  assert(lpParam);

  if (!WriteProcessMemory(hProcess, lpParam, hookedDllPath.c_str(), pathSize,
                          NULL)) {
    perror("WriteProcessMemory");
  }

  HANDLE hTh = CreateRemoteThread(hProcess, NULL, NULL,
                                  (LPTHREAD_START_ROUTINE)pLoadLibraryW,
                                  lpParam, NULL, NULL);
  wcout << "Hooked '" << hookedDllPath << "' in process [" << hProcess
        << "] from thread [" << hTh << "]" << endl;
